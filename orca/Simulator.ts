import { newPoint, newVector, Vec3, vec_add, vec_div, vec_dot, vec_magnitude, vec_magnitude_sq, vec_mul, vec_normalize, vec_sub } from "../math.js";
...
export function computeOrcaLines(unit: Unit, otherUnits: Unit[], dt: number): Line[] {
    ...
    for (let i = 0; i < otherUnits.length; i++) {
        ...
        const line: Line = { point: newPoint(0, 0), direction: newVector(0, 0) };
        let u: Vec3 = newPoint(0, 0); 
...
export function computeNewVelocity(unit: Unit, otherUnits: Unit[], dt: number): Vec3 {
...
function linearProgram1(lines: Line[], lineNo: number, radius: number, optVelocity: Vec3) {
}